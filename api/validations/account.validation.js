...
export default {
    ...
    validatePassword: Joi.object({
        ...
        newPassword: Joi.string().required().empty().regex(/^[a-zA-Z]{6,16}$/).min(minLen).max(maxLen).messages({
        ...
        confirmPassword: Joi.string().required().empty().regex(/^[a-zA-Z]{6,16}$/).min(minLen).max(maxLen).messages({
    ...
    validatePhoneNumber: Joi.object({
        phoneNumber: Joi.string().required().empty().regex(/^[0-9]{10}$/).min(phone).max(phone).messages({
